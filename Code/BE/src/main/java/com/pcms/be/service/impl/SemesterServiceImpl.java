...
public class SemesterServiceImpl implements SemesterService {
    ...
    public boolean validateCapstonePhases(CreatedSemesterRequest createdSemesterRequest) {
        ...
        for (CapstonePhase c : capstonePhases) {
            if (!(!c.getBeginAt().isBefore(createdSemesterRequest.getBeginAt()) && !c.getBeginAt().isAfter(createdSemesterRequest.getEndAt())) || !(!c.getEndAt().isBefore(createdSemesterRequest.getBeginAt()) && !c.getEndAt().isAfter(createdSemesterRequest.getEndAt()))) {
                return false;
            }
        }
        for (int i = 0; i < capstonePhases.size() - 1; i++) {
            ...
            for (int j = i + 1; j < capstonePhases.size(); j++) {
                ...
                if ((currentPhase.getBeginAt().isBefore(equalPhase.getBeginAt()) && currentPhase.getEndAt().isAfter(equalPhase.getBeginAt())) || (currentPhase.getBeginAt().isAfter(equalPhase.getBeginAt()) && currentPhase.getBeginAt().isBefore(equalPhase.getEndAt())) || (currentPhase.getEndAt().isAfter(equalPhase.getBeginAt()) && currentPhase.getEndAt().isBefore(equalPhase.getEndAt()))) {
    ...
    public boolean validDuplicateName(List<CapstonePhase> capstonePhases) {
        Set<String> uniqueNames = capstonePhases.stream().map(CapstonePhase::getName).collect(Collectors.toSet());

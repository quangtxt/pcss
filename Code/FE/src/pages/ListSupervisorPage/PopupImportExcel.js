...
const PopupImportExcel = (props) => {
  ...
  const handleGetDataExcel = async () => {
    ...
    try {
      const response = await fileStore.sendDataExcelSupervisor(formData);
  ...
  const handleDownLoadTemplate = async () => {
    try {
      loadingAnimationStore.showSpinner(true);
      const response = await fileStore.downloadTemplateAccountSupervisor();
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "template-supervisor.xlsx");
